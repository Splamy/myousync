...
pub fn apply_metadata_to_file(path: &Path, tags: &MetadataTags) -> anyhow::Result<()> {
    ...
    let mut album = tag.get_album_info().unwrap_or_else(Album::default);
    ...
    tag.set_comment("youtube_id", tags.youtube_id.to_string());
...
pub fn rebuild_cache(s: &MsState, cache: &mut FileCache) {
    cache.lookup.clear();
    info!("Rebuilding file cache");
    create_cache(&s.config.paths.music, cache);
    if let Some(migrate) = &s.config.paths.migrate {
        info!("Rebuilding migrate cache");
        create_cache(migrate, cache);
    }
    info!("Cache rebuilt with {} entries", cache.lookup.len());
}

fn create_cache(path: &Path, cache: &mut FileCache) {
    cache.lookup.extend(
        WalkDir::new(path)
            .into_iter()
            .filter_map(|p| p.ok())
            ...
            .map(|f| f.into_path())
            .filter_map(|p| multitag::Tag::read_from_path(&p).ok().map(|t| (t, p)))
            .filter_map(|(t, p)| {
                t.get_comment("youtube_id")
                    .map(|y| (YoutubeVideoId::new(y), p))
            }),
    );
}

fn file_tag_matches(path: &Path, video_id: &YoutubeVideoId) -> bool {
    multitag::Tag::read_from_path(path)
        .ok()
        .and_then(|t| t.get_comment("youtube_id"))
        .is_some_and(|y| YoutubeVideoId::new(y) == *video_id)
}

pub fn move_file_to_library(
    s: &MsState,
    path: &Path,
    tags: &MetadataTags,
    cache: &mut FileCache,
    ...
    let clean_album = &tags
        ...
        .album
        .clone()
        .map_or_else(|| clean_title.clone(), |a| sanitize_default(&a));
    ...
    move_file(&s.config.paths, path, &new_path, cache)?;

    apply_permissions(&s.config.paths, &new_path, true);

    cache.lookup.remove(&tags.youtube_id);
    cache.lookup.insert(tags.youtube_id.clone(), new_path);

    Ok(())
}

pub fn delete_file(s: &MsPaths, path: &Path, cache: &mut FileCache) -> anyhow::Result<()> {
...
fn apply_permissions(s: &MsPaths, path: &Path, is_file: bool) {
    if !s.is_sub_file(path) {
        return;
    }

    let set_perm = if is_file {
        &s.file_permissions
    } else {
        &s.dir_permissions
    };

    if let Some(perm) = set_perm {
        if let Err(err) = fs::set_permissions(path, perm.clone()) {
            error!(
        ...
        // If we don't have folder permissions to set -> exit early,
        // as traversing up will only be folders
        if !is_file {
            return;
        }
    }
    ...
    if let Some(parent) = path.parent() {
        apply_permissions(s, parent, false);
    }
}
